            ...
            if (movies) {
                ...
                const makePaginationItem = (
                    ...
                    const paginationItem = $(
                        `<div ${isFinite(content) && typeof content === "number"
                            ? `data-page="${content}"`
                            : ""
                        } class="pagination-item ${isDisabled ? "disabled" : ""
                        }"><span class="${_class ? _class : null}">${content ?? ""
                        }</span></div>`
                    ).on("click", function () {
                        callback.call(this);
                    });
...
const createSeasonItem = (data, isActive) => {
    const seasonItem = $(
        `<div class='season-item text-sm ${isActive ? "active" : null
        }'>Season ${data}</div>`
    );
...
const createActorItem = (data) => {
    ...
    const name = $(
        `<span class='actor-name text-xs text-center mt-1'>${data.name ?? data.character ?? "unknown"
        }</span>`
    );
...
async function database() {
    return await $.get("/js/database/database.json");
}
